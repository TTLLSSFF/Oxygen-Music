  ...
  <div class="local-music-detail">
    <div class="local-music-container">
      ...
      <!-- <RecycleScroller
        ...
        v-slot="{ item, index }"
      > -->
        <div class="list-item" :class="{'list-item-playing': songId == item.id}" @dblclick="play(item.id, index)" @contextmenu.prevent="openMenu($event,item)" v-for="(item, index) in currentSelectedSongs">
